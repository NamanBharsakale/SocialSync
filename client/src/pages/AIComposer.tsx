             ...
             <div className="absolute bottom-2.5 right-2.5 flex items-center gap-3 text-sm">
              <button onClick={()=>setGenerateImage(!generateImage)} className="flex items-center gap-3 bg-red-50 py-2 px-3 rounded-lg">
                ...
                <div className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full transition-colors
                   duration-200 ease-in-out focus:outline-none ${generateImage?"bg-red-500":"bg-slate-200"}`}>

                  <span className={`pointer-events-none size-4 transform translate 
                    translate--y-0.5 rounded-full bg-white transition ${generateImage ? "translate-x-4.5":"translate-x-0.5"}`} />
                </div>
              </button>

              <button onClick={handleGenerate} disabled={loading}
               className="bg-slate-900 hover:bg-slate-800
              text-white flex items-center gap-2 px-4 rounded-lg">
                {loading? (
                  <>
          ...
          <div className="flex flex-wrap justify-center gap-2 ">
            {tones.map((t)=>(
              <button key={t} onClick={()=> setTone(t)} className={`px-4 py-1.5
               rounded-full text-sm transition-all border ${tone === t? "bg-red-500 border-red-500 text-white"
               ...
               overflow-hidden">
                <div className="flex flex-col h-full space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-red-500 bg-red-50 px-2 py-0.5">{gen.tone}</span>
                    </div>
...

          {
            generations.length === 0 && (
              <div className="col-span-full py-20 text-center space-y-2">
                ...
                <p
                className="text-slate-400 text-shadow-mauve-950">No content generated yet.Try generating some content using the AI.</p>
              </div>
            ...
            <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="relative">
                        <TimerIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
